...
type Task = {
    readonly id: number; // readonly : propriété non modifiable
    title: string; 
    description?: string; // présence du '?' rend la propriété facultative
    completed: boolean;
};
...
const foundTask: Task | undefined = aTasks.find(
    task => task.id === 123
);
...
const aCompletedTasks = aTasks.filter(
    task => task.completed
);
